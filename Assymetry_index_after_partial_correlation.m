...
% plot only those with increase during sleep 
for hh=1:nchan
    for kk=1:nchan
        if difsl_wk(kk,hh)>0.1  %% this threshold could change depending on average resutls
...
for q=1:repeats
    ...
    if length(channel)<4
        ...
        for kk=1:length(channel)
            ...
            Assymetry_index_awake(q)=(couple1-couple2)/(couple1+couple2);
        end
        ...
        for kk=1:length(channel)
            ...
            Assymetry_index_sleep(q)=(couple1-couple2)/(couple1+couple2);
        end
