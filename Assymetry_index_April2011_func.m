function [PC]=assymetry_index_pc(timeTosleep, timeEndSleep)
...
figure_temp=[textmeasure ': ' 'average of awake ' resutlscor.filename(1:end-4)]; saveas(gcf, figure_temp, 'fig')
PC.awakeaverage=awake_average;
clear figure_temp
figure; plot2dhead(awake_average, xyz, B); title([textmeasure ': ' 'average of awake ']);
figure_temp=['Lines-'textmeasure ': ' 'average of awake ' resutlscor.filename(1:end-4)];saveas(gcf, figure_temp, 'fig')
clear figure_temp 
%% 
...
set(gca, 'YTickLabel', s); set(gca, 'XTickLabel', s);
axis xy; axis tight; colorbar('location','EastOutside');
figure_temp=[textmeasure ': ' 'average of awake above thr ' resutlscor.filename(1:end-4)]; saveas(gcf, figure_temp, 'fig')
PC.awakeaveragepos=awake_average_pos;
PC.awakethr=threshold_awake;

figure; plot2dhead(awake_average_pos, xyz, B); title([textmeasure ': ''average of awake above threshold ' num2str(threshold_awake)]);
figure_temp=['Lines-'textmeasure ': ' 'average of awake above thr ' resutlscor.filename(1:end-4)]; saveas(gcf, figure_temp, 'fig')
...
axis xy; axis tight; colorbar('location','EastOutside');
figure_temp=[textmeasure ': ' 'average of sleep ' resutlscor.filename(1:end-4)]; saveas(gcf, figure_temp, 'fig')
PC.sleepaverage=sleep_average;

figure; plot2dhead(sleep_average, xyz, B);title([textmeasure ': average of sleep ']);
figure_temp=['Lines-'textmeasure ': ' 'average of sleep ' resutlscor.filename(1:end-4)]; saveas(gcf, figure_temp, 'fig')
%% 
...
set(gca, 'YTickLabel', s); set(gca, 'XTickLabel', s);
axis xy; axis tight; colorbar('location','EastOutside');
figure_temp=[textmeasure ': ' 'average of sleep above thr ' resutlscor.filename(1:end-4)]; saveas(gcf, figure_temp, 'fig')

figure; plot2dhead(sleep_average_pos, xyz, B);title([textmeasure ': ' 'average of sleep above threshold ' num2str(threshold_sleep)]);
figure_temp=['Lines-'textmeasure ': ' 'average of sleep above thr ' resutlscor.filename(1:end-4)]; saveas(gcf, figure_temp, 'fig')
...
stempp=[resultscor.now '-MostCouples-' textmeasure]; % do not delete this!!!!
