...
h=0
 for kl=1:nchan
     for kj=1:nchan
         if (kl~=kj)
             ...
             onesz=find(a)
             if length(onesz)==0
                     points1=squeeze(result1(kl,kj,:)); figure; plot(timeVec2, points1, '*'); title([s(kl) '-' s(kj) 'correlation (blue) & partial correl coeff (red)' ]);axis tight;xlabel('time (hrs)')
                    hold on; points2=squeeze(result2(kl,kj,:));plot(timeVec2, points2, '*r');
                     chan1=cell2mat(s(kl)); chan2=cell2mat(s(kj)); stemp=['correlation-partial correlation' num2str(chan1)  '-' num2str(chan2)]; saveas(gcf, stemp, 'fig'); 
             end
             clear onesz a 
         end
 ...
 result_cor=result1;
stemp=['partial-and-correlation-results' ];
eval(['save ' stemp ' result1 result2 timeVec2 s nchan num_epochs'])
