 ...
 for kl=1:nchan
     for kj=1:nchan
         if (kl~=kj)
             ...
             onesz=find(a)
             if length(onesz)==0
                     points=squeeze(result1(kl,kj,:)); figure; plot(timeVec2, points, '*'); 
                     title([name s(kl) '-' s(kj) 'partial-correlation' ]);axis tight;xlabel('time (hrs)'); 
                     YLim([-1 1]);
                     chan1=cell2mat(s(kl));
                     chan2=cell2mat(s(kj)); 
                     stemp=['partial-correlation-raw' num2str(chan1)  '-' num2str(chan2)]; 
                     saveas(gcf, stemp, 'fig'); 
%                      hold on; points2=squeeze(result1_csd(kl,kj,:)); plot(timeVec2, points2, '*'); title(['Gun07sep09' s(kl) '-' s(kj) 'partial-correlation csd (blue) raw (r) ' ]);axis tight;xlabel('time (hrs)'); YLim([-1 1]);
%                      chan1=cell2mat(s(kl)); chan2=cell2mat(s(kj)); stemp=['partial-correlation-raw-csd-compare' num2str(chan1)  '-' num2str(chan2)]; saveas(gcf, stemp, 'fig'); 
 ...
 clear stemp
 stemp=['partial-correlation-comparison-results' ];
 toc/60
 eval(['save ' stemp ' result1 name timeVec2 s nchan num_epochs'])
